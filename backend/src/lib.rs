...
use crate::cache::{DecryptedCache, DecryptedCacheState, DecryptedPreviewCache, DecryptedPreviewCacheState, HistoryCacheState, wipe_history_cache, write_history_cache, write_settings_cache};
use crate::commands::*;
use crate::crypto::{
    content_hash, parse_recipient, PlaintextPayload,
    EncryptedRecord, HistoryEntry, Vault, VaultState,
};
pub use clipboard_history::AppError;
use clipboard_history::constants::{HISTORY_KEY, HISTORY_STORE_FILE, SETTINGS_KEY, SETTINGS_STORE_FILE};
use clipboard_history::{AppSettings, CopiedObject, CopiedObjectPreview, ObjectContent, ObjectFormat};
use clipboard_rs::{
...
impl ClipboardManager {
    ...
    fn read_clipboard(&self) -> Option<CopiedObject> {
        let ctx = &self.ctx;
        let (content, content_format, thumbnail, formatted_content) = match self.detect_content_type()? {
            ContentFormat::Files => {
            ...
            ContentFormat::Image => {
                ...
                let thumbnail = img.thumbnail(120, 80).and_then(|thumbnail| thumbnail.to_png()).ok().map(|buf| buf.get_bytes().to_vec());
                (
...
impl ClipboardHandler for ClipboardManager {
    fn on_clipboard_change(&mut self) {
        ...
        // To-Do: This could probably be written better, something about this looks ugly
...
fn append_to_history(app: &AppHandle, object: &mut CopiedObject) -> Result<Option<u32>, AppError> {
    ...
    let id = match history.iter().map(|entry| entry.id()).max() {
        Some(max) => max
            .checked_add(1)
            .ok_or_else(|| AppError::Custom("History ID space exhausted. Clear some entries.".into()))?,
        None => 1,
    };
    object.id = id;
...
fn prune_expired(app: &AppHandle) -> Result<(), AppError> {
    ...
    if retention_amount == 0 { return Ok(()); }
    ...
    let now = SystemTime::now()
        ...
        .map(|duration| duration.as_millis() as u64)
        .unwrap_or(0);
    let window = retention_amount
        .saturating_mul(retention_unit.ms_per_unit());
    ...
    history.retain(|item| {
        ...
        let expired = match item {
            ...
            HistoryEntry::Encrypted(encrypted_record) => match identity.as_ref() {
                Some(id) => encrypted_record.decrypt_with_identity(id)
                    .map(|mut payload| {
        ...
        .setup(|app| {
            ...
            thread::spawn(move || {
                ...
                let mut watcher = ClipboardWatcherContext::new()
                    .expect("failed to init clipboard watcher");
